...
save_training_images_interval = 25
ngf = 64
ndf = 64
...
class CycleGAN():
    ...
    def input_setup(self):
        ...
        filenames_A = tf.train.match_filenames_once("../data/horse2zebra/trainA/images/*.jpg")
        self.queue_length_A = tf.size(filenames_A)
        filenames_B = tf.train.match_filenames_once("../data/horse2zebra/trainB/images/*.jpg")
    ...
    def input_read(self, sess):
        ...
        self.fake_images_A = np.zeros((pool_size,1,img_height, img_width, img_channels))
        self.fake_images_B = np.zeros((pool_size,1,img_height, img_width, img_channels))


        self.A_input = np.zeros((max_images, batch_size, img_height, img_width, img_channels))
        self.B_input = np.zeros((max_images, batch_size, img_height, img_width, img_channels))

        for i in range(max_images):
            image_tensor = sess.run(self.image_A)
            if(image_tensor.size == img_size*batch_size*img_channels):
                self.A_input[i] = image_tensor.reshape((batch_size,img_height, img_width, img_channels))

        for i in range(max_images):
            image_tensor = sess.run(self.image_B)
            if(image_tensor.size == img_size*batch_size*img_channels):
                self.B_input[i] = image_tensor.reshape((batch_size,img_height, img_width, img_channels))
    ...
    def save_training_images(self, sess, epoch):
        ...
        if not epoch % save_training_images_interval:
            ...
            for i in range(0,10):
                ...
                imsave(output_path + str(epoch)+"_fakeB_" + str(i)+".jpg",((fake_A_temp[0]+1)*127.5).astype(np.uint8))
                imsave(output_path + str(epoch)+"_fakeA_" + str(i)+".jpg",((fake_B_temp[0]+1)*127.5).astype(np.uint8))
                imsave(output_path + str(epoch)+"_cycA_" + str(i)+".jpg",((cyc_A_temp[0]+1)*127.5).astype(np.uint8))
                imsave(output_path + str(epoch)+"_cycB_" + str(i)+".jpg",((cyc_B_temp[0]+1)*127.5).astype(np.uint8))
                imsave(output_path + str(epoch)+"_inputA_" + str(i)+".jpg",((self.A_input[i][0]+1)*127.5).astype(np.uint8))
                imsave(output_path + str(epoch)+"_inputB_" + str(i)+".jpg",((self.B_input[i][0]+1)*127.5).astype(np.uint8))
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter("../tensorboard/"+now)
            ...
            for epoch in range(sess.run(self.global_step),EPOCHS):
                ...
                if(epoch < EPOCHS) :
                    ...
                    curr_lr = 0.0002 - 0.0002*(epoch-EPOCHS)/EPOCHS
                ...
                for ptr in range(0, max_images):
                    ...
                    print("== iteration time: %s s ==\n " % str(time.time() - i_time))
                ...
                print("== Epoch Time: %s s ==\n" % str(time.time() - epoch_starting_time))
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            ...
            for i in range(0,100):
                ...
                imsave(output_path+"test/"+str(i)+"fakeB.jpg",((fake_A_temp[0]+1)*127.5).astype(np.uint8))
                imsave(output_path+"test/"+str(i)+"fakeA.jpg",((fake_B_temp[0]+1)*127.5).astype(np.uint8))
                imsave(output_path+"test/"+str(i)+"inputA.jpg",((self.A_input[i][0]+1)*127.5).astype(np.uint8))
                imsave(output_path+"test/"+str(i)+"inputB.jpg",((self.B_input[i][0]+1)*127.5).astype(np.uint8))
